...
interface Part3ModeProps {
  onBack: () => void;
  onComplete: () => void;
  part2Topic: string;
}

export default function Part3Mode({ onBack, onComplete, part2Topic }: Part3ModeProps) {
  ...
  const [userResponses, setUserResponses] = useState<Array<{ question: string; transcript: string }>>([]);
  ...
  useEffect(() => {
    loadQuestion();
  }, [currentQuestionIndex, part2Topic]);

  const loadQuestion = async () => {
    setIsLoadingQuestion(true);
    try {
      const backendUrl = getBackendUrl();
      const response = await fetch(`${backendUrl}/ielts/part3/question`, {
        method: 'POST',
        headers: {
  ...
  const processUserAudio = async (audioBlob: Blob) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (currentQuestionIndex < 4) { // Part 3 typically has 4-5 questions
          setCurrentQuestionIndex(prev => prev + 1);
        } else {
          // Part 3 complete, show option to evaluate
  ...
  const evaluatePart3 = async () => {
    ...
    try {
      ...
      userAudioBlobsRef.current.forEach((blob, index) => {
        formData.append('files', blob, `part3_audio_${index}.webm`);
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      <header className="bg-white border-b border-slate-200 py-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-purple-600 rounded-lg flex items-center justify-center text-white font-bold">P3</div>
          ...
          <button
            onClick={onBack}
            className="text-sm font-medium text-slate-500 hover:text-purple-600 flex items-center gap-1"
          >
            <ArrowLeft size={16} />
            Back
          </button>
      ...
      <main className="max-w-4xl mx-auto px-4 mt-8">
        ...
        {!evaluationResults ? (
          <>
            <div className="bg-purple-600 rounded-2xl p-6 text-white shadow-lg mb-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-semibold">Part 3: Discussion</span>
                <span className="text-sm">Question {currentQuestionIndex + 1} of 5</span>
              </div>
              <p className="text-sm opacity-90 mb-2">Discuss abstract ideas related to: <strong>{part2Topic}</strong></p>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-800">Part 3 Evaluation Report</h2>
              <div className="flex items-center gap-2 bg-purple-50 px-4 py-2 rounded-lg border border-purple-100">
                <span className="text-sm text-purple-800 font-medium">Overall Band</span>
                <span className="text-2xl font-bold text-purple-600">{evaluationResults.overall_band}</span>
              </div>
            </div>
